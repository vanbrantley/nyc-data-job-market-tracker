...
MAX_CRAWL_DELAY = 3.5
MIN_SCRAPE_DELAY = 4.0  # seconds between job page requests
...
class BuiltInNYCScraper:
    ...
    def _crawl_index(self, search_url: str) -> list[dict]:
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            try:
                ...
                if not script_tag:
                    log.info(
                        f"Crawl: no JSON-LD on page {page_num} — "
                        f"end of results."
                    )
                    break
    ...
    def _scrape_jobs(self, job_records: list[dict]) -> list[dict]:
        ...
        for i, record in enumerate(job_records):
            ...
            log.info(
                f"Scrape [{i+1}/{len(job_records)}]: sleeping {delay:.2f}s ..."
            )
            time.sleep(delay)
            ...
            try:
                # plain requests.get — more reliable than session for job pages
