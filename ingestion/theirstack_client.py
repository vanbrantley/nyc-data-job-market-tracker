...
# #   Stage 2 — Paid fetch: request only the deduplicated IDs with
# #             blur_company_data=False (1 credit per job). Capped at
# #             PRODUCTION_LIMIT to protect the monthly credit budget.
# #
# # Returns a flat list of Snowflake-ready row dicts:
# #     { "SOURCE": str, "RAW_PAYLOAD": dict, "INGESTED_AT": str }
...
# # 200 credits/month, runs every 3 days (~10 runs/month).
# # 25 credits/run × 10 runs = 250 credits — slightly over the 200 limit.
# # In practice runs will often return fewer than 25 unique jobs,
# # and the high-water mark filter reduces volume on incremental runs.
# # Monitor usage and drop to 20 if consistently hitting the cap.
# PRODUCTION_LIMIT = 25
# # PRODUCTION_LIMIT = 3  # down from 25 for testing pipeline
...
#     On each run the client:
#       1. Sweeps all matching pages for free to collect job IDs
#       2. Deduplicates using (title, location, tech_slugs) fingerprint
#       3. Fetches only the unique IDs via the paid endpoint
#       4. Returns Snowflake-ready rows

#     For incremental runs, pass discovered_at_gte to skip jobs already
#     ingested in previous runs:
#         rows = client.fetch_all(discovered_at_gte="2026-05-20T00:00:00Z")
#     """

#     # Search parameters — locked in after notebook validation.
#     # Targets entry-level / junior data roles in NYC only.
...
#     ) -> list[dict]:
#         """
#         Run the full two-stage pipeline and return Snowflake-ready rows.

#         Args:
#             discovered_at_gte: ISO 8601 UTC timestamp. When provided,
#                 only jobs discovered by TheirStack after this timestamp
#                 are returned. Use this for incremental runs by passing
#                 the MAX(discovered_at) from your Snowflake landing table.

#         Returns:
#             List of dicts with keys SOURCE, RAW_PAYLOAD, INGESTED_AT.
#         """
#         # Stage 1 — free sweep
#         all_jobs = self._free_sweep(discovered_at_gte=discovered_at_gte)
#         log.info(f"Free sweep complete — {len(all_jobs)} total jobs collected.")
...
#         # Stage 3 — cap and extract IDs for paid fetch
#         production_jobs = unique_jobs[:PRODUCTION_LIMIT]
#         production_ids = [job["id"] for job in production_jobs]
#         log.info(
#             f"Paying for {len(production_ids)} jobs "
#             f"(capped at PRODUCTION_LIMIT={PRODUCTION_LIMIT}, "
#             f"{len(unique_jobs) - len(production_ids)} unique jobs left on table)."
#         )

#         if not production_ids:
#             log.info("No new jobs to fetch — returning empty list.")
#             return []

#         # Stage 4 — paid fetch
#         paid_jobs = self._paid_fetch(production_ids)
#         log.info(f"Paid fetch complete — {len(paid_jobs)} jobs returned.")

#         return self._to_snowflake_rows(paid_jobs)
...
#                 **self._SEARCH_BODY,
#                 "blur_company_data": True,
#                 "include_total_results": page == 0,  # only need total on page 0
#                 "limit": FREE_SWEEP_PAGE_SIZE,
#                 "page": page,
#             }
...
#         Rationale: TheirStack parses tech slugs from the job description
#         using a consistent algorithm. The same posting scraped from
#         multiple job boards will produce identical slugs, making this
#         a reliable cross-board deduplication signal.

#         Known edge case: the same job listed as "New York" on one board
#         and "New York, NY" on another will slip through. Accepted
#         tradeoff — rare in practice, complex to solve reliably.
#         """
#         fingerprint_groups: dict[tuple, list[dict]] = defaultdict(list)
...
#         # Keep the first occurrence of each fingerprint group.
#         # TheirStack returns results ordered by discovered_at desc,
#         # so the first occurrence is always the most recently discovered.
#         unique = [group[0] for group in fingerprint_groups.values()]
...
#     def _paid_fetch(self, job_ids: list[int]) -> list[dict]:
#         """
#         Fetch full unblurred job records for the given IDs.
#         Costs 1 credit per job returned.
#         """
...
#         log.info(
#             f"  Paid fetch — requesting {len(job_ids)} jobs "
#             f"({len(job_ids)} credits)."
#         )
...
#         if len(jobs) != len(job_ids):
#             log.warning(
#                 f"  Expected {len(job_ids)} jobs, got {len(jobs)}. "
#                 f"Some listings may have expired or been removed."
#             )
...
#         raise RuntimeError(
#             f"All {MAX_RETRIES} attempts failed. " f"Last error: {last_exception}"
#         )

#     def _to_snowflake_rows(self, jobs: list[dict]) -> list[dict]:
#         """
#         Wrap each raw job dict into the standard Snowflake landing row shape:
#             SOURCE       — identifies this client and search context
#             RAW_PAYLOAD  — complete raw job dict, untouched
